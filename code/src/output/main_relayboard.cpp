...
RelayBoard* relayBoards[NUM_OF_RELAY_BOARDS] = { new RelayBoard(0),
		new RelayBoard(1), new RelayBoard(2), new RelayBoard(3), new RelayBoard(
				4), new RelayBoard(5), new RelayBoard(6), new RelayBoard(7) };
...
void loop() {
	if (receive(recvBuffer, recvBufferSize)) {
		...
		else if (strncmp(recvBuffer, masterBoardIdStr, masterBoardIdStrLen)
				== 0) {
			parsedArgs = sscanf(recvBuffer, "om%d b%d r%d %d",
					&parsedMasterBoardId, &relayBoardId, &relayId, &relayValue);
